      ...
      <div className="card">
        <div className="flex gap-3 mb-4 flex-wrap items-end">
          <label className="flex flex-col gap-1">
            ...
            <input type="date" className="input max-w-[160px]" value={dateRange.from} onChange={(e) => setDateRange({ ...dateRange, from: e.target.value })} />
          </label>
          <label className="flex flex-col gap-1">
            <span className="text-xs text-gray-500">To</span>
            <input type="date" className="input max-w-[160px]" value={dateRange.to} onChange={(e) => setDateRange({ ...dateRange, to: e.target.value })} />
          </label>
          <button onClick={() => setDateRange({ from: '', to: '' })} className="btn-ghost text-sm">Clear</button>
        </div>
          ...
          <div className="space-y-2">
            {logs.map((log) => (
              <div key={log.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-xl">
                <span className="text-xl w-8">{CATEGORY_ICON[log.category] || '📂'}</span>
                <div className="flex-1 min-w-0">
                  <p className="font-medium text-sm truncate">{log.appName || log.url || 'Unknown'}</p>
                  <p className="text-xs text-gray-400">{log.device?.name} · {new Date(log.startTime).toLocaleString()}</p>
                </div>
                <span className="text-sm font-medium text-gray-600 whitespace-nowrap">{fmtDur(log.durationMinutes)}</span>
                <span className="badge-blue capitalize text-xs">{log.category?.replace('_', ' ')}</span>
              </div>
            ))}
          </div>
        )}
