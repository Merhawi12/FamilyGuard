...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([childrenApi.list(), alertsApi.list()]).then(([c, a]) => {
      setChildList(c.data);
      setAlertList(a.data);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="card">
          <h2 className="font-semibold mb-4">Screen Time This Week</h2>
        ...
        <div className="card">
          ...
          {childList.length === 0 ? (
            <div className="text-center py-8">
              ...
              <a href="/children" className="text-blue-500 text-sm hover:underline mt-1 block">Add a child</a>
            </div>
          ) : (
            <div className="space-y-3">
              {childList.map((child) => (
                <div key={child.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                  ...
                  <span className="badge-blue">{child.age ? `Age ${child.age}` : 'No age'}</span>
                </div>
              ))}
            </div>
          )}
