 ...
 * start until the first sync answers — so it is remembered here. Nothing depends
 * on it: every screen reads fine with no name at all.
 */
export async function loadChildName() {
  ...
  if (fromRules) {
    if (fromRules !== cached) {
      cached = fromRules;
      setItem(KEY, fromRules).catch(() => {});
    }
    return fromRules;
  }

  if (cached) return cached;
  cached = await getItem(KEY).catch(() => null);
  return cached;
}
