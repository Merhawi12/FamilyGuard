 ...
 *
 * This harness sits in the shared package, and that package deliberately has no
 * node_modules of its own — it declares every library as a peerDependency so the
 * only copy in a bundle is the platform project's. Node resolves imports from
 * the importing *file*, so `axios` and `socket.io-client` would be looked for
 * beside this script and not found.
 *
 * The project that ran `npm run test:e2e` is the cwd, and that is what gets
 * anchored: here for the harness's own requires, and via the loader for the app
 * modules it drives. Same rule the two metro.config.js files apply — one
 * explicit node_modules, and no walking up the tree to the repo root, where the
 * web apps resolve a different React.
 *
 * Either platform can run it. React Native is stubbed out entirely, so this
 * proves the JS contract and the wire protocol rather than anything
 * platform-specific, and `npm run test:e2e:child` at the repo root runs it once,
 * from the Android project.
 */
const PROJECT_ROOT = path.resolve(process.env.CHILD_E2E_PROJECT || process.cwd());
if (!existsSync(path.join(PROJECT_ROOT, 'node_modules'))) {
  console.error(
    `No node_modules in ${PROJECT_ROOT}\n\n`
    + 'Run this from a platform project:\n'
    + '  npm --prefix apps/child-app/android run test:e2e\n'
    + 'or point CHILD_E2E_PROJECT at one.'
  );
  process.exit(1);
}
...
const run = async () => {
  ...
  step('Bedtime and schedule rules decide the lock');
  {
    ...
    /*
     * A parent's pause outranks every scheduled reason, and has to work in the
     * two places the clock-driven reasons cannot reach: with no screen-time rule
     * at all, and with the rule switched off. Switching the schedule off is how
     * a parent lifts a bedtime; it must not be how a child lifts a block.
     */
    const paused = { reason: 'blocked_by_parent', since: '2026-08-17T12:00:00.000Z' };
    check('a parent block locks the device',
      lockState(base, 0, wednesdayAt('10:00'), paused).reason === 'blocked_by_parent');
    check('a parent block locks even with no rule at all',
      lockState(null, 0, wednesdayAt('10:00'), paused).blocked === true);
    check('a parent block outranks a switched-off rule',
      lockState({ ...base, isActive: false }, 0, wednesdayAt('10:00'), paused).blocked === true);
    check('a parent block beats the daily limit as the stated reason',
      lockState(bedtime, 500, wednesdayAt('22:00'), paused).reason === 'blocked_by_parent');
    check('an unrecognised block payload still locks',
      lockState(base, 0, wednesdayAt('10:00'), { since: null }).reason === 'blocked_by_parent');
    check('no block means the ordinary rules decide',
      lockState(base, 0, wednesdayAt('10:00'), null).blocked === false);
  ...
  {
    /**
     * A window covering all but one minute of the day, anchored so that the one
     * minute is never the minute this runs in.
     *
     * It used to be the fixed `00:00 → 23:59`, with a comment claiming the
     * assertion therefore did not depend on when it ran. It did: `withinWindow`
     * treats a non-wrapping window as `start <= minute < end`, so 23:59 itself
     * fell outside it and this check failed for exactly one minute a night. It
     * duly failed on a run started at 23:59.
     *
     * Starting at the current minute and ending one minute earlier makes the
     * window wrap, which puts `now` at the very start of it and the single
     * excluded minute a full day away. `start === end` would be rejected as a
     * zero-length window that blocks nothing, which is why this is 1439 minutes
     * rather than 1440.
     */
    const hhmm = (minute) => {
    ...
    const nowMinute = startedAt.getHours() * 60 + startedAt.getMinutes();
    await call('PUT', `/screen-time/${childId}`, {
      token: parentToken,
      body: {
