...
const _state = {
  queued: 0,
  uploaded: 0,
  dropped: 0,      // entries discarded because the queue was full
  lastUploadAt: null,
  lastError: null,
};
 ...
 * If the handset is then linked to a sibling — the ordinary reason a parent
 ...
 *
 * The native side already collapses repeat lookups within its flush window, but
 * two windows can carry the same domain, and a batch that failed to upload is
 * put back. Merging on (domain, firstSeen) means neither produces a duplicate:
 * the counts add up instead.
 */
function mergeIntoQueue(visits) {
  ...
  for (const visit of visits) {
    ...
    if (_queue.length >= MAX_QUEUE) {
      // Oldest first: a device offline for days should still report what the
 ...
 * failed upload is retried on the next pass rather than lost. The queue is
 * persisted after every change, which is what lets it survive the app or the
 * device restarting mid-backlog.
 */
  ...
  try {
    while (_queue.length > 0) {
      ...
      if (res.data?.rejected) {
        // Not fatal, but not silent either — the server dropped entries it could
        // not parse, and that should be visible in the device log.
...
/** Take whatever the native buffer holds right now and queue it. */
export async function collectNow() {
  try {
    await VpnControl.flushWebHistory?.();
  } catch (err) {
    console.warn('[webHistory] native flush failed:', err.message);
  }
}
...
export async function startWebHistory() {
  ...
  _sub?.remove();
  _sub = VpnControl.onWebVisits?.(async (visits) => {
...
/** Test seam: feed visits in as though the native module had emitted them. */
export const __testing = {
  ingest: async (visits) => { mergeIntoQueue(visits); await persistQueue(); },
  queue: () => [..._queue],
